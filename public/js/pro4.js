...
GEClient.prototype.TrackCallBack = function (CUID, SESID, sesCookieExists, isEx, cBuster) {
   
  
    if (isEx) { this.InTracking = false; return; }
    //create cookie
    this.CID = CUID;
    this.CacheBuster = cBuster == "" ? null : cBuster;
    
    this.TrackCallBackFired = true;
...
GEClient.prototype.Track = function (optUrl, igGeObject) {
    ...
    var root = this.GetAnalyticsUrl();// this.GetCDNUrl();
    ...
    //bust/temp
    //pixelUrl += "&cb=" + new Date().getTime();
 
    this.AddScript(pixelUrl, null, null, function () {
        self.TrackCallBackFired = true;
        console.log("error loading tracking service");
    });
...
GEClient.prototype.LoadWelcome = function (country, culture, currency, callback) {
    ...
    if (!this.TrackCallBackFired && GlobalE.MPH.Get(MPH.K.AllowClientTracking, MPH.T.Bool)) {//make sr
        //wait for track callback
        setTimeout(function () {
            self.LoadWelcome(country, culture, currency, callback);
        }, 200);
    }
    else {
...
GEClient.prototype.LoadShippingSwitcher = function (country, culture, currency, as, callback, callOnBeforeCallback, isModal) {
    ...
    if (!this.TrackCallBackFired && GlobalE.MPH.Get(MPH.K.AllowClientTracking, MPH.T.Bool)) {//make sr
        //wait for track callback
        setTimeout(function () {
            self.LoadShippingSwitcher(country, culture, currency, as, callback, callOnBeforeCallback, isModal);
        }, 200);
    }
    else {
...
//FOR PROD ONLY - TO OVERCOME OLD MERCHANTS THAT DO NOT UPDATE THE MERCHANT APP SETTINGS
//GEClient.prototype.GetAnalyticsUrl = function () { 
//    return "https://services.global-e.com/";
//}


GEClient.prototype.GetAnalyticsUrl = function () {
 
    var url = GlobalE.MPH.Get(MPH.K.AnalyticsUrl, MPH.T.String);
    if (url == null) {
        url = this.GetCDNUrl();
    }
